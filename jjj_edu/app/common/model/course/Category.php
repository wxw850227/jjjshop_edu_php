...
class Category extends BaseModel
{
    ...
    {
        $model = new static;
        if (!Cache::get('course_category_' . $model::$app_id)) {
            $CourseModel = new Course();
            $courseCountSql = $CourseModel->field(['count(course_id)'])
                ->where('category_id', '=', "c.category_id")->buildSql();
            $data = $model->alias('c')
                ->with(['image'])
                ->field("*,$courseCountSql AS courseCount")
            ...
            Cache::set('course_category_' . $model::$app_id, compact('all', 'tree'));
        }
        return Cache::get('course_category_' . $model::$app_id);
    }
