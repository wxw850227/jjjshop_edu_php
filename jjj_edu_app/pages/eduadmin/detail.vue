	...
	<view class="product-detail pr">
		<view :catchtouchmove="true">
			<scroll-view scroll-y="true" class="scroll-Y scroll-box" :style="'height:' + scrollviewHigh + 'px'"
				...
				<view class="mb24 bg-white p20 " v-if="detail.lecturer">
					...
					<scroll-view scroll-x="true" style="white-space: nowrap;">
						<view class="d-b-c lecturer-box" v-for="v in detail.lecturer" :key="v"
							@click="gotoPage('/pages/knowledge/lecturer/detail?lecturer_id=' + v.lecturer_id)">
							<view class="lecturer ww100 d-s-c">
								<view class="lecturer-image">
									<image :src="v.image && v.image.file_path" mode="aspectFill">
									</image>
								</view>
								<view class="flex-1">
									<view class="f24 gray3 mb10">{{ v.name }}</view>
									<view class="label-list ww100" style="margin-bottom: 0;">
										<!-- <view class="label-item" v-for="(item, index) in detail.label" :key="index"> -->
										<view class="label-item">
											{{ v.label && v.label.length > 0 && v.label[0] }}
										</view>
									</view>
								</view>
							</view>
				...
				<view class="bg-white">
					...
					<view v-if="activeName == 1" class="section-list">
						<template v-if="detail.section && detail.section.length > 0">
							<template v-for="(item, index) in detail.section" :key="index">
								<view class="section-item" @click="gotoLive(item)">
									<view class="d-b-c flex-1 box-s-b" style="height: 100rpx;">
										<view class="zjContent">
											<image class="video-icon" src="/static/image/zjIcon.png" mode=""></image>
											<view>
												<view class="title">{{ item.title }}</view>
												<view class="tearchName">讲师:{{ item.lecturer && item.lecturer.name }}</view>
												<view class="tearchTime">直播时间:{{ item.start_time_text }}</view>
											</view>
										</view>
										<template v-if="true">
											<view class="is-see is-see0" v-if="item.status == 0">未开始</view>
											<view class="is-see is-see1" v-else-if="item.status == 1">直播中</view>
											<view class="is-see is-see2" v-else-if="item.status == 2">回放</view>
										</template>
									</view>
								</view>
							</template>
						</template>
						<empty desc="暂无课表" v-else /> 
					</view>
			...
			<view class="btns-wrap">
				...
				<template v-if="detail.is_pay == 0">
					<button class="buy" v-if="detail.payStatus > 0" @click="getConcat">免费领取</button>
					<button class="buy" v-if="detail.payStatus == 0" @click="getToConcat">免费领取</button>
				</template>
				<template v-else>
					<button class="buy" @click="toPay()"
						v-if="detail.payStatus == 0">￥{{ detail.course_price }}</button>
					<button class="buy" @click="getConcat" v-else>已购买</button>
				</template>
			</view>
		</view>
		<!--app分享-->
...
<script>
	...
	import share from '../knowledge/course/part/share.vue';
	...
	export default {
		...
		methods: {
			gotoLive(item) {
				let url;
				if (this.detail.is_pay == 1 && this.detail.payStatus == 0) {
					uni.showToast({
						icon: 'none',
						title: '未购买，请先购买再观看'
					});
				...
				this.gotoPage(`/pages/common/webview?url=${url}`);
			},
			getConcat() {
				this.$refs.concatRef.openShow(this.detail);
			},
			getToConcat() {
				let self = this;
				let params = {
				...
				self._post('course.Order/eduBuy', params, function(res) {
					self.$refs.concatRef.openShow(self.detail);
				});
			...
			/*打开客服*/
			openMaservice() {
				this.gotoPage('/pages/user/qa/index');
				// let self = this;
				// self._get(
				// 	'index/mpService', {},
				// 	function(res) {
			...
			toPay() {
				...
				self.gotoPage(`/pages/order/confirm?course_id=${self.course_id}&type=eduadmin`);
				// self.isPay = true;
			},
			//分享按钮
			showShare() {
	...
	.section-list {
		...
		.section-item {
			...
			.is-see {
				...
				&.is-see1 {
					background: rgba(248, 156, 49, 0.25);
					color: #F2473F;
				}
