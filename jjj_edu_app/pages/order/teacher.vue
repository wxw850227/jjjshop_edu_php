			...
			<view class="order-item" v-for="(item,index) in listData" :key="index"
				...
				<view class="d-b-c item-bottom">
					...
					<view class="d-e-c flex-1">
						<view class="order-btn full" @click.stop="gotoPage(`/pages/order/cashier?order_id=${item.order_id}&order_type=10&type=eduadmin`)" v-if="item.pay_status.value == 10">去支付</view> 
						<template  v-else-if="item.order_status && item.order_status.value == 30 && item.is_comment == 0">
							<view class="order-btn full" @click.stop="gotoEdu(item)">立即学习</view> 
							<view class="order-btn" @click.stop="gotoPage(`pages/order/evaluate?order_id=${item.order_id}`)">去评价</view>
						</template>
						<template  v-else>
							<view class="order-btn full" @click.stop="gotoEdu(item)">立即学习</view> 
							<view class="order-btn" @click.stop="gotoPage('/pages/knowledge/index')">更多课程</view>
						</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			gotoEdu(item){
				if(item.product && item.product[0] && item.product[0].course && item.product[0].course.course_id){
					this.gotoPage(`/pages/eduadmin/detail?course_id=${item.product[0].course.course_id}`)
				}
			}
		}
	}
