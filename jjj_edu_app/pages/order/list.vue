...
<template>
	<view>
		<zsTab :data="tabList" :activeName="order_type" @chang-tab="changeType" />
		<!-- <view class="top-tabbar">
			<view class="tab-item" :class="{active:order_type == 0}" @click="changeType(0)">全部订单</view>
			<view class="tab-item" :class="{active:order_type == 10}" @click="changeType(10)">课程订单</view>
			<view class="tab-item" :class="{active:order_type == 20}" @click="changeType(20)">试题订单</view>
		</view> -->
		<view class="order-list">
			<view class="order-item" v-for="(item,index) in listData" :key="index"
				...
				<view class="d-b-c item-bottom">
					<view class="redF24 f24">{{item.order_status.text}}</view>
					<view class="d-e-c flex-1">
						<view class="order-btn full" @click.stop="gotoPage(`pages/order/evaluate?order_id=${item.order_id}`)" v-if="item.order_status && item.order_status.value == 30 && item.is_comment == 0">去评价</view>
						<view class="order-btn full" @click.stop="gotoPage('/pages/knowledge/index')" v-else>更多课程</view>
						<view class="order-btn" @click.stop="deleteFunc(item.order_id,index)" v-if="item.pay_type.value == 10">删除订单</view>
					</view>
				</view>
			</view>
			<view class="">
...
<script>
	...
	export default {
		...
		methods: {
			...
			changeType(n) {
				if (this.order_type == n.value) {
					return
				}
				this.order_type = n.value;
				this.listData = [];
				this.page = 1;
				this.getData();
			}
