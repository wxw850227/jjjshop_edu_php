<template>
	<!--
    	描述：组件-选择练习
    -->
	<el-dialog title="选择练习" v-model="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false"
		:close-on-press-escape="false" width="900px">
		<div class="common-seach-wrap">
			<el-form :inline="true" size="small" :model="formInline" class="demo-form-inline">
				<el-form-item label="练习分类">
					<el-select v-model="formInline.category_id" placeholder="请选择练习分类">
				...
				<el-form-item label="练习名称">
					<el-input placeholder="请输入练习名称" v-model="formInline.search">
					</el-input>
				</el-form-item>
				<el-form-item label="">
		...
		<div class="lecturer-content">
			<div class="table-wrap">
				<el-table size="small" :data="lecturerList" border style="width: 100%" highlight-current-row
					v-loading="loading" @selection-change="tableCurrentChange">
					<el-table-column width="70" label="练习图片">
						<template #default="scope">
							<img :src="scope.row.image.file_path" width="30" height="30" />
						</template>
					</el-table-column>
					<el-table-column prop="title" label="练习名称"></el-table-column>
					<el-table-column prop="category.name" width="100" label="练习分类"></el-table-column>
...
<script>
	...
	export default {
		...
		props: ['isTrain', 'excludeIds', 'islist'],
		watch: {
			isTrain: function(n, o) {
				if (n != o) {
					if (n) {
						...
						if (res.code == 1) {
							...
							if (self.excludeIds && typeof(self.excludeIds) != 'undefined' && self.excludeIds.length >
								...
								res.data.list.data.forEach(item => {
									if (self.excludeIds.indexOf(item.paper_id) > -1) {
										item.noChoose = false;
									} else {
										item.noChoose = true;
									}
